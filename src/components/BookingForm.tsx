...
export default function BookingForm() {
  ...
  const handleWhatsAppBooking = () => {
    const dateStr = formData.pickup_date ? formData.pickup_date.toLocaleString() : 'À préciser';
...
Date: ${dateStr}`;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const dateStr = formData.pickup_date ? formData.pickup_date.toLocaleString() : 'À préciser';
    const subject = i18n.language === 'fr' ? 'Nouvelle réservation' : 'New Booking';
...
${formData.notes ? `Notes: ${formData.notes}` : ''}`;
    ...
    window.location.href = `mailto:contact@presticar.fr?subject=${encodedSubject}&body=${encodedBody}`;
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <DatePicker
                  ...
                  placeholderText={t('booking.date.placeholder')}
                  required
                />
              </div>
